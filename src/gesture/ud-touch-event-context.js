...
class UDTouchEventContext{
    ...
    @DECORATORS.serializable(true)
    @DECORATORS.field({type:Types.BOOLEAN,desc:'alt 键是否被按下',value:false})
    altKey(){}

    @DECORATORS.serializable(true)
    @DECORATORS.field({type:Types.BOOLEAN,desc:'shift 键是否被按下',value:false})
    shiftKey(){}

    @DECORATORS.serializable(true)
    @DECORATORS.field({type:Types.BOOLEAN,desc:'ctl 键是否被按下',value:false})
    ctlKey(){}
    
    @DECORATORS.field({type:Types.ARRAY(UDTouch),desc:'所有当前接触触摸平面的触点的 Touch 对象',value:[]})
    touches(){}

    @DECORATORS.field({type:Types.ARRAY(UDTouch),desc:'触摸起始于当前事件的目标 element 上，并且仍然没有离开触摸平面的触点的 Touch 对象',value:[]})
    targetTouches(){}
    // // altKey = field({type:Types.CLASS(UDAttribute),serializable:false}); //键盘 alt 键是否被按下
    // // altKey= DECORATORS.serializable(true)(createAttribute('alt键是否被按下',false,Types.BOOLEAN)); //键盘 alt 键是否被按下
    // ctlKey=createAttribute('ctl键是否被按下',false,Types.BOOLEAN); //键盘 ctl 键是否被按下
    // shiftKey=createAttribute('shift键是否被按下',false,Types.BOOLEAN); //键盘 shift 键是否被按下
