...
- 编辑器的对象树开发
  ...
  - [`进行中`]*让对象树的对象支持多选、拖动操作（可以拖动到对象树任意位置）
  - [`已完成`]让对象支持可编辑属性的动态变化
    - [`已完成`]当对象处于不同的位置的时候，其自身的某些属性可被编辑器忽略（重置为默认值），比如，当元素被放置在相对定位容器如"行"、"列"中时，x,y属性不可见。
  - [`进行中`]*对象支持设置属性Attribute的可选单位列表，表示这个属性可以允许被设置几种单位
  - [`已完成`]*UDLayoutRow 布局对象开发
  - [`进行中`]UDLayoutCol 布局对象开发
  - [`进行中`]UDBindingFor 循环绑定对象开发
    - 支持在容器内部循环绑定子元素
    - 需要考虑子元素的样式代码复用的问题（避免子元素重复的内联样式冗余）
- 左侧工具箱开发
  - [`已完成`]列表展示所有可用的工具
  ...
  - [`进行中`]面板中不同数据类型的处理逻辑和UI控件
    ...
    - [`进行中`]数字输入框功能
      ...
      - [`进行中`]*数字输入框支持单位和控制框轮流展示
        - [`进行中`]*能够根据属性内部的"可选属性列表"，自动在数字框里显示单位切换按钮，单位可以进行切换
    - [`已完成`]slider组件功能
    - [`已完成`]组件绑定值同步功能
    - [`已完成`]颜色选择框
    - [`已完成`]文件上传框
...
- 特殊组件
  - 前端数据类组件
    - 可以在前端定义一些数据、变量，用于存储数据、或者作为绑定数据的来源
  - 前端数据绑定组件
    - 可以让用户自定义前端的一些动态渲染规则，比如：“循环创建”组件
- 遗留问题解决
  - Page、Container等可视容器,以及Text等含内容的组件，其高度、宽度，是否要支持`自动`、`最小宽度`、`最大宽度`等属性
    - 背景
